<!-- ============================================================
  Student.vue — NEW DESIGN 1:1 (worked example for Claude Code)
  Drop-in replacement for src/components/center/Student.vue
  • <script> = UNCHANGED byte-for-byte (data/methods/API/store/i18n เดิมทั้งหมด:
    server pagination tableOptions/totalStudents/getStudentList, search watcher,
    showAddNewStudent/clickEditStudent/clickDeleteStudent, ฟอร์ม+อัปโหลดรูป, showRetrieveStudent).
  • <template> เฉพาะส่วน LIST ถูกรื้อเป็นดีไซน์ใหม่:
    - v-data-table-server  ->  toolbar มาตรฐาน (.grid-toolbar: ค้นหา + รีเฟรช + เพิ่ม อยู่แถวเดียวกัน)
      + ตาราง .idt (วนหัวจาก StudentListHeaders เดิม) + footer pager (ผูก tableOptions.page เดิม)
    - v-dialog ฟอร์มเพิ่ม/แก้ไข + ลบ + finish = คงไว้ทั้งหมด (เปิดด้วย showAddNewStudent แทน activator)
  • คลาส .grid-toolbar/.idt/.scard/.pager-* มาจาก istar-pages.css (ต้อง import global)
